...
static uint16_t ADCValue[ADC_SCANNUM];
...
static void adc_single_channel_init(uint32_t channel)
{
    ...
    //Enable the channel
    ADC_RegularChannelConfig(ADC1, channel, channel, ADC_Samctl_239_5);

    ADC_ANY_NUM_Config(ADC1, ADC_SCANNUM);
...
static void adc_filter(void)
{
    uint16_t channel;
    uint16_t cntFilter;
    uint32_t lADCFilterValue[ADC_SCANNUM] = {0, 0, 0, 0};
...
{
    //Stop Conversion
    if(RESET != DMA_GetITStatus(DMA_IT_TC))
    {
